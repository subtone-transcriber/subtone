#!/usr/bin/env bash
# toMidi.sh — Strict & Non-Forgiving Audio Event Preprocessing Pipeline
set -euo pipefail

# Immediate failure trap for shell execution
trap 'echo "❌ Strict execution failure at line $LINENO" >&2' ERR

echo "=== 🎹 Deploying Audio Event Extraction Engine (Strict Mode) ==="

ENV_DIR=".venv_tomidi"

# Force eager loading for python lazy_loader packages globally in the subshell
export PYTHONLAZYLOADING=0
# Suppress package deprecation warnings (e.g. pkg_resources / resampy / scikit-learn)
export PYTHONWARNINGS="ignore::UserWarning,ignore::DeprecationWarning"

# Safe pattern matching helper to prevent 'set -e' crashes on compgen
has_files() {
    local pattern="$1"
...
# ------------------------------------------------------------------------------
if [ -d "$ENV_DIR" ]; then
    echo "⚡ Activating pristine environment '$ENV_DIR'..."
    # shellcheck disable=SC1091
    source "$ENV_DIR/bin/activate"
else
    echo "📦 Initializing pristine virtual environment and installing dependencies..."
    ...
    if [[ "${OS}" == "Darwin" ]]; then
        if command -v brew &> /dev/null; then
            echo "📦 Installing PortAudio dependency via Homebrew..."
        ...
        if command -v apt-get &> /dev/null; then
            ...
            dpkg -s portaudio19-dev &> /dev/null || MISSING+=("portaudio19-dev")

            if [ ${#MISSING[@]} -gt 0 ]; then
                echo " Installing missing Linux system dependencies: ${MISSING[*]}"
                sudo DEBIAN_FRONTEND=noninteractive apt-get update && sudo DEBIAN_FRONTEND=noninteractive apt-get install -y "${MISSING[@]}"
            fi
        elif command -v dnf &> /dev/null; then
            MISSING=()
            rpm -q portaudio-devel &> /dev/null || MISSING+=("portaudio-devel")
            rpm -q libsndfile &> /dev/null || MISSING+=("libsndfile")
            command -v ffmpeg &> /dev/null || MISSING+=("ffmpeg")
            if [ ${#MISSING[@]} -gt 0 ]; then
    ...
    echo "⚙️  Installing Python package toolchain..."
    pip install --upgrade pip "setuptools<82" wheel
    pip install "cython<3" "numpy<2" lazy_loader

    if [[ "${OS}" == "Darwin" && "${ARCH}" == "arm64" ]]; then
        pip install pyaudio coremltools basic-pitch pretty_midi "librosa>=0.10.2" soundfile mido essentia
    else
        pip install pyaudio basic-pitch pretty_midi "librosa>=0.10.2" soundfile mido essentia
    fi
fi

# Assert environment integrity explicitly
python -c "import os; os.environ['PYTHONLAZYLOADING']='0'; import librosa; assert hasattr(librosa, 'pyin'); import pretty_midi, basic_pitch, essentia"

# ------------------------------------------------------------------------------
# 2. Dynamic Python Event Extractor
# ------------------------------------------------------------------------------
cat << 'EOF' > run_tomidi.py
import os
import sys
import json
import warnings
...
# ------------------------------------------------------------------------------
if [ $# -eq 0 ]; then
    python run_tomidi.py
else
    for ITEM in "$@"; do
        if [ -f "$ITEM" ]; then
            # Strict Audio File Whitelist Check
            EXT="${ITEM##*.}"
            EXT_LOWER=$(echo "$EXT" | tr '[:upper:]' '[:lower:]')
            if [[ ! "$EXT_LOWER" =~ ^(mp3|wav|flac|m4a|aac|ogg|aiff|opus)$ ]]; then
                echo "⏭️  Skipping non-audio file: '$ITEM'"
                continue
            fi

            FILENAME=$(basename "$ITEM")
            STEM="${FILENAME%.*}"
            STEM_DIR="stems_${STEM}"
            MIDI_DIR="./midi/${STEM}"

            # Safely check if MIDI directory already exists AND contains both .midi AND .json maps
            if [ -d "$MIDI_DIR" ] && has_files "$MIDI_DIR/*.json" && has_files "$MIDI_DIR/*.midi"; then
                echo ""
                echo "⏭️  Skipping audio file: '$ITEM' (MIDI directory '$MIDI_DIR' already contains MIDI & JSON maps)"
                continue
            fi

            echo ""
            echo "🎵 Processing audio file: '$ITEM'"

            if [ -x "./split.sh" ]; then
                ./split.sh "$ITEM"
            elif [ -f "./split.sh" ]; then
                bash ./split.sh "$ITEM"
            else
                echo "❌ Error: split.sh not found in current directory." >&2
                exit 1
            fi

            if [ -d "$STEM_DIR" ]; then
                if ! has_files "$STEM_DIR/*.wav"; then
                    echo "⚠️ Warning: Stem folder '$STEM_DIR' is empty. Cleaning up..."
                    rm -rf "$STEM_DIR"
                    continue
                fi
                ...
                python run_tomidi.py "$STEM_DIR"

                INPUT_DIR=$(dirname "$ITEM")
                if [ -d "$MIDI_DIR" ]; then
                    find "$MIDI_DIR" -maxdepth 1 -name "*.json" -exec cp -f {} "$INPUT_DIR/" \; 2>/dev/null || true
                fi

                echo "🧹 Cleaning up intermediate stems folder: '$STEM_DIR'..."
                rm -rf "$STEM_DIR"
            else
                echo "❌ Error: Expected stem output directory '$STEM_DIR' was not created." >&2
                exit 1
            fi

        elif [ -d "$ITEM" ]; then
            echo ""
            echo "📁 Processing stem directory: '$ITEM'"

            if ! has_files "$ITEM/*.wav"; then
                echo "⚠️ Warning: No .wav files found in directory '$ITEM'. Skipping..."
                continue
            fi

            python run_tomidi.py "$ITEM"
        else
            echo "❌ Error: '$ITEM' is neither a valid file nor directory." >&2
            exit 1
        fi
    done
fi