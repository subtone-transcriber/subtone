...
if [ -d "$ENV_DIR" ]; then
    ...
    if [[ "${OS}" == "Darwin" && "${ARCH}" == "arm64" ]]; then
        pip install pyaudio coremltools basic-pitch pretty_midi "librosa>=0.10.2" soundfile mido
    else
        pip install pyaudio basic-pitch pretty_midi "librosa>=0.10.2" soundfile mido
    fi
fi

# Assert environment integrity explicitly (checking librosa.pyin)
python -c "import os; os.environ['PYTHONLAZYLOADING']='0'; import librosa; assert hasattr(librosa, 'pyin'); import pretty_midi, basic_pitch"
...
if [ $# -eq 0 ]; then
    ...
    for ITEM in "$@"; do
        if [ -f "$ITEM" ]; then
            ...
            # Check if MIDI directory already exists and contains files
            if [ -d "$MIDI_DIR" ] && [ "$(ls -A "$MIDI_DIR" 2>/dev/null)" ]; then
                echo ""
                echo "⏭️  Skipping audio file: '$ITEM' (MIDI directory '$MIDI_DIR' already exists and is populated)"
                continue
            fi
